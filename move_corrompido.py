...
def criar_estrutura_de_pastas(caminho_base, nome_arquivo, origem):
    ...
    if camera:
        ...
        if tamanho_arquivo == 0:
            destino_corrompido = os.path.join(caminho_base, 'CORROMPIDO', nome_arquivo)
            os.makedirs(os.path.dirname(destino_corrompido), exist_ok=True)
            shutil.move(origem, destino_corrompido)
            print(f"Arquivo {nome_arquivo} movido para a pasta corrompido.")
            return
